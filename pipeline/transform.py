...
from pipeline.utils_function.db_connector import db_connection
from pipeline.utils_function.read_sql import read_sql_file
...
class Transform(luigi.Task):
    ...
    def run(self):
        ...
        try:
            ...
            fct_daily_orders_query = read_sql_file(
                file_path = f'{DIR_TRANSFORM_QUERY}/fct_daily_orders.sql'
            )
            ...
            fct_order_delivery_query = read_sql_file(
                file_path = f'{DIR_TRANSFORM_QUERY}/fct_order_delivery.sql'
            )
            
            fct_customer_reviews_query = read_sql_file(
                file_path = f'{DIR_TRANSFORM_QUERY}/fct_customer_reviews.sql'
            )
        ...
        try:
            ...
            # Transform to final.fct_daily_orders
            query = sqlalchemy.text(fct_daily_orders_query)
            session.execute(query)
            logging.info("Transform to 'final.fct_daily_orders' - SUCCESS")
            
            # Transform to final.fct_order_payments
            query = sqlalchemy.text(fct_order_payments_query)
            session.execute(query)
            logging.info("Transform to 'final.fct_order_payments' - SUCCESS")

            # Transform to final.fct_order_delivery
            query = sqlalchemy.text(fct_order_delivery_query)
            session.execute(query)
            logging.info("Transform to 'final.fct_order_delivery' - SUCCESS")
            
            # Transform to final.fct_customer_reviews
            query = sqlalchemy.text(fct_customer_reviews_query)
            session.execute(query)
            logging.info("Transform to 'final.fct_customer_reviews' - SUCCESS")
